...
# Final Model
For my final model, I added two new features, calories and minutes, in addition to the features used in my baseline model. I also engineered three new features, which are ingredients per step, calories per ingredient, and minutes per step. Ingredients per step shows how many ingredients are in each step, which can be a good predictor of average rating because for example, recipes that have too many complicated ingredients in each step might receive lower ratings than recipes with simpler and fewer ingredients because many people might enjoy working with simple amount of ingredients more. Calories per ingredients shows how much calories are in each ingredient, which can also be a good predictor of average rating because people might enjoy recipes with high-calorie ingredients that are more filling, and recipes with higher calories are likely to be more tasty too, hence they might be highly rated by users. Minutes per step shows how much time, in minutes, each step takes, which can be a valuable predictor of average rating because how much time and effort spent for a recipe can be an important factor that can influence users' ratings of the recipe. For example, recipes that are quick and don't take a lot of effort to make might be more likely to receive higher ratings from users because generally people enjoy things that are quick and easy to do. 

The modeling algorithm I chose is random forest regressor, and this algorithm is very good at handling complex patterns and reducing overfitting compared to a single decision tree, for example. The hyperparameters that ended up performing the best are max_depth = 4 (tree depths) and n_estimators = 15 (number of trees), in which I used GridSearchCV to find the best combination of those two hyperparameters. Overall, my final model's performance is slighly better than my baseline model as both the train RMSE and test RMSE have reduced to roughly 0.63873 and 0.64043. My final model includes more features that help improve the accuracy in predicting average rating.

# Fairness Analysis
My choice of Group X is recipes with 10 or fewer ingredients and Group Y is recipes with more than 10 ingredients. My evaluation metric is RMSE. My null hypothesis is that the model is fair, RMSE of Group X and Group Y are roughly the same. My alternative hypothesis is that the model is unfair, RMSE of Group Y is higher than Group X. My choice for the test statistic is the difference in RMSE between Group Y and Group X. My choice for significant level is 0.05, and my resulting p-value is 0.86, which is way higher. So my test fails to reject the null hypothesis. The conclusion is that there's no strong evidence that RMSE of Group Y is higher than RMSE of Group X. 
